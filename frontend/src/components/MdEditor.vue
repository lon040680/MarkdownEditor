...
<script>
import { readFile, writeFile, exportToPdf, exportToHtml } from '../api/fileApi'
...
export default {
  ...
  methods: {
    ...
    async saveFile() {
      ...
      try {
        await writeFile(this.currentFile, this.content)
    ...
    async doExportPdf() {
      ...
      if (!filePath) return
      try {
        await exportToPdf(this.content, filePath)
