...
export async function exportToPdf(content, path, mdPath = '') {
  const res = await fetch(`${API_BASE}/export/pdf`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ content, path, md_path: mdPath })
  })
  if (!res.ok) {
    const err = await res.json()
...
export async function saveImage(directory, filename, base64Data) {
  const res = await fetch(`${API_BASE}/file/image`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ directory, filename, data: base64Data })
  })
  if (!res.ok) {
    const err = await res.json()
    throw new Error(err.detail || 'Failed to save image')
  }
  return res.json()
}
